...
export const calculateAvailability = async (
  ...
  for (let i = 0; i < savedVisualizationsIds.length; i++) {
    ...
    const handleDeletedViz = (value: string) => {
      skip = true;
      if (availabilityVisId === value.substring(value.indexOf(':') + 1)) {
        console.log('should remove availability vis id');
      }
    };
    // Fetches data for visualization
    const visData = await fetchVisualizationById(http, visualizationId, handleDeletedViz);
