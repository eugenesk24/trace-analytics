...
import './application_analytics/app_analytics.scss';
import {
  EuiBadge,
  EuiButton,
  EuiContextMenuItem,
...
import { getClearModal } from './application_analytics/helpers/modal_containers';
import { pageStyles, UI_DATE_FORMAT } from '../../../common/constants/shared';
import { ApplicationListType } from '../../../common/types/app_analytics';
import { AvailabilityType } from './application_analytics/helpers/types';
...
interface AppTableProps extends AppAnalyticsComponentDeps {
  ...
  clearStorage: () => void;
  moveToApp: (id: string, type: string, integration: string) => void;
}
...
export function AppTable(props: AppTableProps) {
  ...
  const [isEmptyCreatePopoverOpen, setIsEmptyCreatePopoverOpen] = useState(false);
  ...
  const createPopoverButton = (
    <EuiButton
      data-test-subj="appAnalyticsCreatePopoverButton"
      iconType="arrowDown"
      iconSide="right"
  ...
  const emptyCreatePopoverButton = (
    <EuiButton
      data-test-subj="appAnalyticsCreatePopoverButton"
      iconType="arrowDown"
      iconSide="right"
      onClick={() => setIsEmptyCreatePopoverOpen(!isEmptyCreatePopoverOpen)}
    >
      Create
    </EuiButton>
  );

  const actionsPopoverItems: ReactElement[] = [
    <EuiContextMenuItem
      key="rename"
  ...
  const createPopoverItems: ReactElement[] = [
    <EuiContextMenuItem
      key="createApp"
      onClick={() => {
        window.location.assign('#/application_analytics/create/app');
    ...
    <EuiContextMenuItem
      key="createDummyApp"
      onClick={() => {
        window.location.assign('#/application_analytics/create/dummy');
        setIsCreatePopoverOpen(false);
      }}
    >
      Create Dummy
    ...
    if (value.color === 'loading') {
      ...
      return (
        <EuiLink
          ...
          onClick={() => moveToApp(record.id, 'createSetAvailability', 'dummy')}
        >
          Set Availability
        </EuiLink>
      );
  ...
  const tableColumns = [
    {
      ...
      render: (value, record) => (
        <EuiLink
          ...
          href={`#/application_analytics/${record.name}/${record.id}`}
        >
              ...
              <EuiPageContentHeaderSection>
                <EuiFlexGroup gutterSize="s">
                  <EuiFlexItem>
                    ...
                    >
                      <EuiContextMenuPanel items={actionsPopoverItems} />
                    </EuiPopover>
                  </EuiFlexItem>
                  <EuiFlexItem>
                    <EuiPopover
                      panelPaddingSize="none"
                      button={createPopoverButton}
                      isOpen={isCreatePopoverOpen}
                      closePopover={() => setIsCreatePopoverOpen(false)}
                    >
                      <EuiContextMenuPanel items={createPopoverItems} />
                    </EuiPopover>
                  </EuiFlexItem>
                </EuiFlexGroup>
              </EuiPageContentHeaderSection>
              ...
              <>
                ...
                <EuiFlexGroup justifyContent="center">
                  <EuiFlexItem grow={false}>
                    <EuiPopover
                      panelPaddingSize="none"
                      button={emptyCreatePopoverButton}
                      isOpen={isEmptyCreatePopoverOpen}
                      closePopover={() => setIsEmptyCreatePopoverOpen(false)}
                    >
                      <EuiContextMenuPanel items={createPopoverItems} />
                    </EuiPopover>
                  </EuiFlexItem>
