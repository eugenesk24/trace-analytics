...
import {
  filtersToDsl,
  PanelTitle,
} from '../../../trace_analytics/components/common/helper_functions';
...
export function DummyApplication(props: AppDetailProps) {
  const {
    pplService,
    dslService,
    timestampUtils,
  ...
  const updateAvailabilityVizId = (vizs: VisualizationType[]) => {
    if (!vizs.map((viz) => viz.savedVisualizationId).includes(application.availabilityVisId)) {
      updateApp(appId, { availabilityVisId: '' }, 'editAvailability', 'dummy');
    }
  };
