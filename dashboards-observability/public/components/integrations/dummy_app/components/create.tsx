...
interface CreateAppProps extends AppAnalyticsComponentDeps {
  ...
  createApp: (app: ApplicationType, type: string, integration: string) => void;
  updateApp: (
    appId: string,
...
export const CreateDummy = (props: CreateAppProps) => {
  const {
    parentBreadcrumbs,
    chrome,
    http,
  ...
  const onUpdate = () => {
    ...
    updateApp(existingAppId, appData, 'update', 'dummy');
  };
          ...
          <EuiPageHeader>
            <EuiPageHeaderSection>
              <EuiTitle data-test-subj="createPageTitle" size="l">
                <h1>{editMode ? 'Edit' : 'Create'} DUMMY application</h1>
              </EuiTitle>
            </EuiPageHeaderSection>
          </EuiPageHeader>
