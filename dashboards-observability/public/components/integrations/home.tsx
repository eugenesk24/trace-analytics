...
import { Application } from './application_analytics/components/application';
import { CreateApp } from './application_analytics/components/create';
import { CreateDummy } from './dummy_app/components/create';
...
import {
  ...
  isNameValid,
  removeTabData,
} from './application_analytics/helpers/utils';
import {
  CUSTOM_PANELS_API_PREFIX,
  CUSTOM_PANELS_DOCUMENTATION_URL,
} from '../../../common/constants/custom_panels';
import { DummyApplication } from './dummy_app/components/application';
...
export const Home = (props: HomeProps) => {
  ...
  const moveToApp = (id: string, type: string, integration: string) => {
    window.location.assign(
      `${last(parentBreadcrumbs)!.href}application_analytics/${integration}/${id}`
    );
  ...
  const createPanelForApp = (
    ...
    return http
      ...
      .then((res) => {
        updateApp(applicationId, { panelId: res.newPanelId }, type, integration);
      })
      .catch((err) => {
        setToast(
  ...
  const createApp = (application: ApplicationType, type: string, integration: string) => {
    ...
    return http
      ...
      .then(async (res) => {
        createPanelForApp(res.newAppId, application.name, type, integration);
  ...
  const updateApp = (
    appId: string,
    updateAppData: Partial<ApplicationType>,
    type: string,
    integration: string
  ) => {
    const requestBody = {
    ...
    return http
      ...
      .then((res) => {
        if (type === 'update') {
          ...
          clearStorage();
          moveToApp(res.updatedAppId, type, integration);
        }
  ...
  return (
    <div>
      ...
      <Switch>
        ...
        <Route
          exact
          path={'/application_analytics/create/app'}
          render={(routerProps) => (
            <CreateApp
              dslService={dslService}
              pplService={pplService}
        ...
        <Route
          exact
          path={'/application_analytics/edit/:id+'}
          render={(routerProps) => (
            <CreateApp
              dslService={dslService}
              pplService={pplService}
        ...
        <Route
          exact
          path={'/application_analytics/create/dummy'}
          render={(routerProps) => (
            <CreateDummy
              dslService={dslService}
              pplService={pplService}
        ...
        <Route
          exact
          path={'/application_analytics/app/:id+'}
          render={(routerProps) => (
            <Application
              disabled={false}
        ...
        <Route
          exact
          path={'/application_analytics/dummy/:id+'}
          render={(routerProps) => (
            <DummyApplication
              disabled={false}
              appId={decodeURIComponent(routerProps.match.params.id)}
